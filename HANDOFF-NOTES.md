...
#32 — Embedded sheol terminal pane (owner asked "whatchu think"): recommendation given — a REAL terminal emulator inside the app needs SwiftTerm (MIT, mature, the only sane embed on macOS); doable as a right-side ledger pane that is closable in-app, with "open in iTerm instead" as the escape hatch. Cheaper interim: AppleScript-position an iTerm window snapped beside ours. Decide when the ledger surface (#26) gets built — SwiftTerm adds a dependency but delivers exactly the owner's ask.
