...
--
-- The whole body is wrapped in try…on error…end try so that an app which
-- refuses to be resized/moved (or exposes no scriptable front window) fails
-- SILENTLY — never a focus-stealing dialog that would block Leader Key input.
on run argv
	try
	...
	on error
		-- swallow: a failed resize must never block Leader Key with a dialog
	end try
end run
