...
**Agent-awareness (a safety cue).** Because the ✕ can end a session that's running an AI agent, every row flags a detected agent CLI with a glyph just left of the ✕ — a 🦀 for **Claude Code**, plus a registry for codex / grok / gemini / deepseek / kimi / qwen / aider / opencode / goose / cursor. It's matched by the session's command (`claude.exe`) or the iTerm foreground job, so it fires **whether or not the tool is installed** — the instant one runs, the row warns you before you close it. Extend it by adding a line to `agentTable` in `StatusItem.swift`. A leading green **`$|`** marks iTerm-hosted rows (a dim `▪` marks non-iTerm apps and detached spirits). Each section also has a **`＋ New`** item to spawn a fresh tmux-protected window / iTerm window, and the snapshot **Save / Restore / ☠ Banish-all** block sits at the very bottom, away from the per-session controls.
