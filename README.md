...
**Agent-awareness (a safety cue).** Because the ✕ can end a session that's running an AI agent, every row flags a detected agent CLI with a mark just left of the ✕ — a drawn **spark** (Anthropic orange) for **Claude Code**, a **rosette** for **Codex**, an angular **slash** for **Grok** (all stylized functional indicators, drawn in code — the monochrome two adapt to light/dark), and a glyph for the rest of the registry (gemini / deepseek / kimi / qwen / aider / opencode / goose / cursor). Detection scans the session's command **and the whole tty process tree**, so an agent behind a wrapper (e.g. `caffeinate` or a `sourcekit-lsp` / `node` foreground) is still caught — and it fires **whether or not the tool is installed**. Extend it by adding a line to `agentTable` in `StatusItem.swift`. A leading green **`$|`** marks iTerm-hosted rows (a dim `▪` marks non-iTerm apps and detached spirits). Each section also has a **`＋ New`** item that spawns a fresh tmux-protected / iTerm window **in the background** (so the ledger stays open for making several), and the snapshot **Save / Restore / ☠ Banish-all** block sits at the very bottom, away from the per-session controls.
